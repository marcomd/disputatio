...
## Corpus Check
- 59 files · ~87,243 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 803 nodes · 895 edges · 58 communities (49 shown, 9 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 10 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d05eeacb`
...
7. `Disputatio: Implementation Plan` - 13 edges
8. `Claude Code — Headless / Non-Interactive Mode (research, 2026-06-05)` - 12 edges
9. `Disputatio` - 11 edges
10. `OpenAI Codex CLI — Headless / Automation Mode (research, 2026-06-05)` - 11 edges
...
Cohesion: 0.09
Nodes (27): continuePrompt(), DebateOutcome, EVIDENCE_PHASES, EvidenceSummary, execFileAsync, finalizePrompt(), finalizeRetryHint(), gitLock (+19 more)
...
Cohesion: 0.14
Nodes (15): ParticipantSpec, usage(), BIN_OVERRIDABLE, BinResolution, DEFAULT_BIN, execFileAsync, findAlternatives(), InitDeps (+7 more)
...
Cohesion: 0.17
Nodes (10): Architecture, Development Style, Git And Workspace Hygiene, graphify, Non-Negotiable Invariants, Private area, Project Context, Runtime And Commands (+2 more)
...
Cohesion: 0.11
Nodes (18): Adapters, Design documents, Disputatio, How it works (v0), How to add a new adapter, How to Run, Install and use, Known v0 limitations (next steps) (+10 more)
...
Cohesion: 0.08
Nodes (23): [0.0.1] — Kaizen MVP v0, [0.0.2] — Agent tooling & knowledge graph, [0.0.3] — Hardening after the first real-repo run, [0.0.4] — `doctor` preflight (M0, canary half), [0.0.5] — Process-group kill + first three-vendor repo-grounded run, [0.0.6] — Per-participant reasoning `effort` in `debate.yaml`, [0.1.0] — `respondeo`: the judge stage (first scholastic-protocol step), [0.2.0] — portable config + installable `disputatio` binary (+15 more)
...
Cohesion: 0.12
Nodes (16): 1. Scope, 2. Architecture: separate Transport from Normalization, 3. The output contract — three levels, 4.1 Claude Code (`claude`) — rich JSON envelope, 4.2 OpenAI Codex (`codex`) — rich JSONL stream, 4.3 Google Antigravity (`agy`) — text-only, 4.4 GitHub Copilot CLI (`copilot-cli`) — JSONL stream, 4. Per-CLI adapter specs (grounded in canary) (+8 more)
...
Cohesion: 0.12
Nodes (13): args, DEFAULT_JUDGE, DEFAULT_SPECS, ev, execFileAsync, input, loadLatestRespondeo(), participants (+5 more)

### Community 47 - "Community 47"
Cohesion: 0.29
Nodes (8): AgentResult, Participant, allHealthy(), diagnose(), Diagnosis, excerpt(), formatDiagnoses(), runDoctor()

### Community 48 - "Community 48"
Cohesion: 0.29
Nodes (6): Binary and install, Canary: read-only file read, GitHub Copilot CLI headless notes, Headless invocation, JSONL shape and success classification, Read-only mechanism
...
Cohesion: 0.11
Nodes (17): 1. Why process metrics, and not just quality + cost, 2. The anti-circularity rule (read this before adding any metric), 3.1 Error amplification, 3.2 Coordination overhead, 3.3 Redundancy, 3.4 Efficiency, 3.5 Failed reviews (reliability), 3.6 "Judge edits" — the term, made precise (+9 more)

### Community 53 - "Community 53"
Cohesion: 0.18
Nodes (10): is_error, num_turns, permission_denials, result, subtype, total_cost_usd, type, usage (+2 more)

### Community 54 - "Community 54"
Cohesion: 0.25
Nodes (7): Finding 1 — shell-less adapters cannot see git at all in a worktree, Finding 2 — a signal-killed turn is undiagnosable, Finding 3 — `ranCommands` needs a capability flag, not just a count, Finding 4 — the redactio was denied a command it tried to run, Real run, 2026-08-04 — shell-less adapters × worktree isolation, Run shape, What this run says about priority
...
Cohesion: 0.18
Nodes (10): duration_api_ms, is_error, num_turns, permission_denials, session_id, stop_reason, total_cost_usd, usage (+2 more)

### Community 57 - "Community 57"
Cohesion: 0.25
Nodes (7): Fixed in v0.9.0, Note for the next timeout, Real run, 2026-08-11 — the redactio timeout, and why it looked like a lost run, Rejected: an interactive "continue or exit?" prompt, Three distinct defects, What happened, What the killed turn actually cost
...
## Knowledge Gaps
- **536 isolated node(s):** `name`, `version`, `description`, `type`, `license` (+531 more)
